...
var STALE_METER_THRESHOLD_MS = 7 * 24 * 60 * 60 * 1000;
var metersData = [];
var statusData = null;
...
function renderDefaultView() {
  ...
  var html = renderMeterGrid(activeMeters);
...
function renderMeterGrid(meters) {
  ...
  for (var i = 0; i < meters.length; i++) {
    html += '<div class="col-md-4 col-sm-6">' + createMeterPanel(meters[i], false) + '</div>';
  }
  html += '</div>';
  return html;
}
...
function renderStaleMetersSection(meters) {
  ...
  html += '    <h3 class="meter-section-title"><span class="glyphicon glyphicon-warning-sign" aria-hidden="true"></span> 未更新のメーター</h3>';
  html += '    <p class="meter-section-subtitle">1週間以上更新されていないデバイス</p>';
  html += '  </div>';
  html += '  <div class="panel panel-warning stale-meters-panel">';
  html += '    <div class="panel-body">';
  html += '      <div class="row">';
  for (var i = 0; i < meters.length; i++) {
    html += '<div class="col-md-4 col-sm-6">' + createMeterPanel(meters[i], true) + '</div>';
  }
  html += '      </div>';
  html += '    </div>';
  html += '  </div>';
...
function createMeterPanel(meter, isStale) {
  ...
  html += '      <div class="meter-panel-title">';
  html += '        <strong>' + deviceName + '</strong>';
  if (isStale) {
    html += '        <span class="label label-warning stale-meter-badge">7日以上未更新</span>';
  }
  html += '      </div>';
...
function isStaleMeter(meter) {
  ...
  var lastUpdated = new Date(meter.last_updated);
...
function renderCharts() {
  for (var i = 0; i < metersData.length; i++) {
    (function(meter) {
      if (isStaleMeter(meter)) {
        if (charts[meter.device_id]) {
          charts[meter.device_id].destroy();
          delete charts[meter.device_id];
        }
        return;
      }
...
$(function() {
  ...
  $('#time-scale-select').on('change', function() {
