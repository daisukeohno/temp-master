...
The frontend is a Vite + React 18 + TypeScript SPA in `switchbot-dashboard/switchbot-frontend/`.

**Recommended for development:** run the Vite dev server, which proxies `/api` and `/healthz` to `http://localhost:8000`:
...
**To verify the production setup** (backend serving the built SPA), build the frontend and symlink the build output:
...
The built frontend is served at `http://localhost:8000/` and the API docs at `http://localhost:8000/docs`.
...
- Charts: Recharts SVG line charts (temperature in Celsius, tooltip shows `°C`)
- Refresh Data button triggers data reload; Download Backup button downloads the SQLite file
- Stale meters (no update for >7 days) are separated into the "未更新のメーター" section
- Theme selector in the navbar: Light / Dark / Dracula / Solarized / High Contrast.
  Switching changes colors immediately and the choice persists across reloads (`localStorage` key `temp-master-theme`)
...
- Frontend: Vite + React 18 + TypeScript, TanStack Query (30s polling), Recharts, Tailwind CSS (built to `switchbot-frontend/dist/`)
